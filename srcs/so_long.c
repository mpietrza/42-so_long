...
static int	ft_key_hook(int keycode, t_data *data, t_map *map)
{
	if (keycode == ESC)
		ft_exit(data);
	else if (keycode == W || keycode == ARR_UP)
		ft_move(data, map, 'y', UP);
	else if (keycode == A || keycode == ARR_LEFT)
		ft_move(data, map, 'x', LEFT);
	else if (keycode == S || keycode == ARR_DOWN)
		ft_move(data, map, 'y', DOWN);
	else if (keycode == D || keycode == ARR_RIGHT)
		ft_move(data, map, 'x', RIGHT);
	if (map->arr[data->pos_y][data->pos_x] == 'E')
		ft_win(data);
	return (0);
}
